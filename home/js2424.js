...
function getGameUrlWithParams(gameUrl) { 
  const lang = getParam("lang"), username = getParam('us'), userId = getParam('i'), telegramLink = getParam('lk'), params = new URLSearchParams(); 
...
function parseProfileFromUrl() { 
  const username = getParam('us'), userId = getParam('i'), telegramLink = getParam('lk'), profileBtn = document.getElementById("profileBtn"), profileInfo = document.getElementById("profileInfo"), profileName = document.getElementById("profileName"); 
...
function applyTranslations(language) { 
  const t = translations[language] || translations.fr; // FIXED: Removed broken image reference
...
function checkLanguageAndRedirect() { 
  const language = getParam("lang"), telegramLink = getParam('lk'); 
  if (!language) { 
...
function loadMode() { 
  const savedTheme = localStorage.getItem('theme') || "dark-mode", lang = getParam("lang") || 'fr', t = translations[lang] || translations.fr; // FIXED: Removed broken image reference
  document.body.className = savedTheme; 
  document.getElementById("modeText").textContent = savedTheme === 'dark-mode' ? t.modeDark : t.modeLight; 
...
function toggleMode() { 
  const lang = getParam("lang") || 'fr', t = translations[lang] || translations.fr, body = document.body; // FIXED: Removed broken image reference
  if (body.classList.contains("dark-mode")) { 
    body.classList.replace("dark-mode", "light-mode"); 
    document.getElementById('modeText').textContent = t.modeLight; 
    document.querySelector("#modeToggle i").className = "fas fa-sun"; 
    saveMode("light-mode"); 
  } else { 
    body.classList.replace('light-mode', "dark-mode"); 
    document.getElementById('modeText').textContent = t.modeDark; 
...
function populateGames() { 
  const gamesGrid = document.getElementById('gamesGrid'), lang = getParam("lang") || 'fr', t = translations[lang] || translations.fr; // FIXED: Removed broken image reference
  gamesGrid.innerHTML = ''; 
  gamesData.forEach(game => { 
    const gameCard = document.createElement('div'); 
    gameCard.className = "game-card"; 
    gameCard.onclick = event => handleGameClick(game.url, event); 
    const categoryText = game.category === "1win bet" ? t.category1win : t.categoryOther; 
    gameCard.innerHTML = `<div class="game-image" style="background-image: url('${game.image}');"><div class="game-overlay"><div class="play-btn" aria-label="Play ${game.name}"><i class="fas fa-play"></i></div></div></div><div class="game-info"><div class="game-name">${game.name}</div><div class="game-category">${categoryText}</div></div>`; 
...
document.addEventListener('DOMContentLoaded', () => {
    const licenseOverlay = document.getElementById('licenseOverlay'), 
          ...
          licenseTimer = document.getElementById('licenseTimer'), 
          body = document.body;
    let isAppInitialized = false;
    
    // *** FUNCTION: Starts and manages the countdown timer ***
    ...
    function lockPage() {
        ...
        translateLicenseOverlay(); // Make sure overlay is translated
    }
    ...
    function checkLicenseStatus() {
        ...
        if (validUntil && new Date().getTime() < parseInt(validUntil)) {
            unlockPage();
            startCountdown(parseInt(validUntil)); // Start countdown on page load
        } else {
            localStorage.removeItem('license_valid_until');
            lockPage();
        }
    ...
    licenseForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok && data.valid && data.durationMs) {
                ...
                unlockPage();
                startCountdown(expirationTime); // Start countdown on successful activation
            } else {
